...
>) => {
  return (
    <section className="flex flex-col gap-2 rounded-4xl border border-primary/40 bg-primary/5 p-4">
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_minmax(0,auto)] items-baseline gap-x-2 gap-y-1">
        ...
        <div className="col-start-2 row-start-1 flex min-w-0 max-w-full justify-self-end text-right text-xs leading-none">
          <Link
            className="flex min-w-0 max-w-full items-center gap-1 text-foreground transition-colors hover:text-foreground/80"
            {...link}
          >
            {link.children}
